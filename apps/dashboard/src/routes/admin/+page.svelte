<script lang="ts">
  ...
  const settingHints: Record<string, string> = {
    ...
    TEXT_MARKERS_CSAM: 'Comma or newline separated hashtags/topic tags that produce a csam-suspected block verdict.',
    TEXT_MARKERS_NSFW: 'Comma or newline separated hashtags/topic tags that produce an NSFW warning verdict.',
  ...
  const multilineSettings = new Set(['TEXT_MARKERS_CSAM', 'TEXT_MARKERS_NSFW']);
          ...
          {#each settings as setting}
            {#if setting.key !== 'MODERATION_PROVIDER'}
              <label>
                ...
                {#if multilineSettings.has(setting.key)}
                  <textarea bind:value={setting.value} rows="4" autocomplete="off"></textarea>
                {:else}
